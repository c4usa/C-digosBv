...
void combatesIniciales(char nombres[][50], int puntos[], int total) { int i; for (i = 0; i < total; i += 2) { int ganador; int knockout; int indiceGanador; printf("\nCombate entre %s y %s\n", nombres[i], nombres[i + 1]); printf("Ingrese 1 si gana %s o 2 si gana %s: ", nombres[i], nombres[i + 1]); ganador = leerEntero(1, 2); printf("¿Fue por knockout? (1 = Sí, 0 = No): "); knockout = leerEntero(0, 1); if (ganador == 1) { indiceGanador = i; } else { indiceGanador = i + 1; } if (knockout == 1) { puntos[indiceGanador] = puntos[indiceGanador] + 3; } else { puntos[indiceGanador] = puntos[indiceGanador] + 1; } } }

int desempate(int indices[], int count, char nombres[][50]) { while (count > 2) { int a = indices[0]; int b = indices[1]; int winner; printf("\nDesempate entre %s y %s\n", nombres[a], nombres[b]); printf("Ingrese 1 si gana %s o 2 si gana %s: ", nombres[a], nombres[b]); winner = leerEntero(1, 2); if (winner == 1) { indices[0] = a; } else { indices[0] = b; } // desplazar los demás { int i; for (i = 2; i < count; i++) { indices[i - 1] = indices[i]; } count = count - 1; } } return count; }
